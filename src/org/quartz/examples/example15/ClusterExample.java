...
public class ClusterExample {
	...
	public void run(boolean inClearJobs, boolean inScheduleJobs) throws Exception {
		...
		if (inScheduleJobs) {
			...
			int count = 1;
			JobDetail job = JobBuilder.newJob(org.quartz.examples.example15.SimpleRecoveryJob.class).withIdentity((new StringBuilder()).append("job_").append(count).toString(), schedId)
			...
			count++;
			job = JobBuilder.newJob(org.quartz.examples.example15.SimpleRecoveryJob.class).withIdentity((new StringBuilder()).append("job_").append(count).toString(), schedId).requestRecovery()
					.build();
			...
			sched.scheduleJob(job, trigger);
			count++;
			job = JobBuilder.newJob(org.quartz.examples.example15.SimpleRecoveryStatefulJob.class).withIdentity((new StringBuilder()).append("job_").append(count).toString(), schedId)
					.requestRecovery().build();
			trigger = (SimpleTrigger) TriggerBuilder.newTrigger().withIdentity((new StringBuilder()).append("triger_").append(count).toString(), schedId)
	...
	private static Logger _log = LoggerFactory.getLogger(org.quartz.examples.example15.ClusterExample.class);
